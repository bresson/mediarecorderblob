...
const stopVideo = document.querySelector('button#stopVideo');
...
recordButton.addEventListener('click', () => {
  if (recordButton.textContent === 'Start Recording') {
    ...
    if (mediaRecorder?.state === 'inactive') {
      console.log('mediaRecorder is inactive and about to start() ');
      mediaRecorder.start();
    } else {
      console.log(
        'mediaRecorder is undefined and about to  startRecording() ',
        mediaRecorder
      );
      startRecording();
    }
  } else {
    console.log('stopped recording fork');
    stopRecording();
    recordButton.textContent = 'Start Recording';
    playButton.disabled = false;
    downloadButton.disabled = false;
...
function startRecording() {
  ...
  try {
    // let fakeStream = null;
    // setTimeout(() => {
    //   fakeStream = window.stream;
    //   console.log('fakestream', fakeStream);
    // }, 2000);
...
function handleSuccess(stream) {
  ...
  gumVideo.srcObject = webcamStream;
...
// what happens if stream is stopped during recording?
